...
from Model import Convolution_ResNet
import torch
...
class Train:
    def __init__(self):
        ...
        self._net = Convolution_ResNet().cuda()
        ...
        self._log = SummaryWriter(log_dir="/root/project/My_projects/CNN/CNN_ResNet/runs")
        self._lossfn = nn.CrossEntropyLoss() 
        ### 交叉熵损失自带One HOT 因此在数据处理时不需要 转成onehot也不需要转成浮点数！！！！！！！！！！！！！！！！！！
    
    def __call__(self):
        
        self._net.load_state_dict(torch.load("/root/project/My_projects/CNN/CNN_ResNet/weights/res_net.pt"))
        ...
        for _epoch in range(100000000000000000):    
            ...
            for _index, (_data, _label) in enumerate(self._train_dataset):
                ...
                print(_loss.detach())
                print(_loss.detach().item())
                print(_loss)
                # print("loss", _loss_sum)
            ...
            torch.save(self._net.state_dict(), "/root/project/My_projects/CNN/CNN_ResNet/weights/res_net.pt")
